...
namespace mc
{
// Global debug mode flag for GPU operations (host and device accessible)
#if defined(__CUDACC__)
static __constant__ bool g_debug_mode_device = false;
#endif
...
add_node_geometry(mqi::node_t<R>*  node,
    ...
    if (g_debug_mode_device) printf("Adding geometry node.. : Node --> %p, number of children --> %d\n", node, n_children);
...
add_node_scorers(mqi::node_t<R>*         node,
    ...
    if (g_debug_mode_device) printf("Adding scorers node.. : Node --> %p, number of children --> %d\n", node, n_scorers);
...
void
upload_node(mqi::node_t<R>* c_node, mqi::node_t<R>*& g_node) {
    if (mc::g_debug_mode) printf("Uploading data to node.. : Allocated node --> %p\n", g_node);
    ...
    if (c_node->n_scorers > 0) {
        ...
        if (c_node->scorers[0]->score_variance_) {
            if (mc::g_debug_mode) printf("Score variance turn on\n");
