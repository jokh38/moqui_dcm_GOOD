...
public:
    ...
    create_machine(std::string machine_name, std::string mc_code, int gantryNum) {
        ...
        if (debug_mode_) {
            ...
            std::cout << "Machine name: " << model << " (Same as selected gantry number)" << std::endl;
        }
        ...
        // Set debug mode for treatment machine
        if (tx_machine_ != nullptr) {
            tx_machine_->debug_mode = debug_mode_;
        }
    ...
    /// Returns a list of beam names present in the plan file (excluding SETUP beams).
    std::vector<std::string>
    get_beam_names() {
        auto                     beam_sequence = (*mqi_ds_)(seq_tags_->at("beam"));
        std::vector<std::string> beam_names;

        std::cout << "=== Filtering beams in RT Plan (excluding SETUP) ===" << std::endl;
        int beam_index = 0;

        for (const auto& beam : beam_sequence) {
            std::vector<std::string> tmp;
            beam->get_values("BeamName", tmp);
            std::string beam_name = !tmp.empty() ? mqi::trim_copy(tmp[0]) : "UNKNOWN";

            std::cout << "Beam[" << beam_index << "] Name: '" << beam_name << "'";

            // Filter by BeamName: exclude beams named "SETUP" (case-insensitive)
            std::string beam_name_upper = beam_name;
            std::transform(beam_name_upper.begin(), beam_name_upper.end(),
                          beam_name_upper.begin(), ::toupper);

            if (beam_name_upper == "SETUP") {
                std::cout << " → EXCLUDED (SETUP beam)" << std::endl;
                beam_index++;
                continue;  // Skip SETUP beams
            }

            std::cout << " → INCLUDED (Treatment beam)" << std::endl;
            beam_names.push_back(beam_name);
            beam_index++;
        }

        std::cout << "=== Total treatment beams: " << beam_names.size() << " ===" << std::endl;
        return beam_names;
    }

    /// Returns the number of beams present in the plan file (excluding SETUP beams).
    int
    get_num_beams() {
        auto beam_sequence = (*mqi_ds_)(seq_tags_->at("beam"));
        // Count 'treatment' beam only, exclude 'setup' beam
        int count = 0;
        for (const auto& beam : beam_sequence) {
            // Filter by BeamName: exclude beams named "SETUP" (case-insensitive)
            std::vector<std::string> tmp;
            beam->get_values("BeamName", tmp);
            if (!tmp.empty()) {
                std::string beam_name_upper = mqi::trim_copy(tmp[0]);
                std::transform(beam_name_upper.begin(), beam_name_upper.end(),
                              beam_name_upper.begin(), ::toupper);
                if (beam_name_upper == "SETUP") {
                    continue;  // Skip SETUP beams
    ...
    get_beamline(S beam_id) {
        if (debug_mode_) {
            ...
            if (mtype_ == 1) std::cout << "RT Plan type: RT ION" << std::endl;
            std::cout << "Beam ID: " << beam_id << std::endl;
        }
