...
public:
    ...
    tps_env(const std::string input_name) : x_environment<R>() {
        ...
        /// Read debug mode first
        this->debug_mode = parser.get_bool("DebugMode", false);
        /// Global parameters
        ...
        if (use_absolute_path) {
            ...
            this->logfile_dir = parser.get_string("logFilePath", "");
        } else {
            this->parent_dir = parser.get_string("ParentDir", "");
        ...
        // Print condensed summary in normal mode
        if (!this->debug_mode) {
            std::cout << "GPU ID: " << this->gpu_id << std::endl;
            std::cout << "Inserted seed: " << master_seed;
            if (master_seed < -1) {
                std::cout << " (random seed)" << std::endl;
            } else {
                std::cout << std::endl;
            }
            std::cout << "Gantry number : " << this->selectedGantryNumber << std::endl;
            if (!parent_dir.empty()) std::cout << "Patient directory: " << parent_dir << std::endl;
        ...
        tx = new mqi::treatment_session<R>(dcm_.plan_name, machineName, referenceMCName, this->selectedGantryNumber, this->debug_mode);
    ...
    read_dcm_dir() 
    {
        ...
        if (this->debug_mode) std::cout << "Reading DICOM directory.. : DICOM directory name --> " << dicom_dir << std::endl;
        ...
        if (dcm.n_plan > 0)
        {
            ...
            // Read RT Plan header information for DICOM export
            if (this->debug_mode) {
                std::cout << "Reading RT Plan header information from " << dcm.plan_name << " .." << std::endl;
            } else {
                std::cout << "RT Plan header information from     " << std::endl;
                std::cout << "  " << dcm.plan_name << std::endl;
            }
            gdcm::Reader plan_reader;
            plan_reader.SetFileName(dcm.plan_name.c_str());
            if (plan_reader.Read()) {
                const gdcm::DataSet& plan_ds = plan_reader.GetFile().GetDataSet();
                gdcm::StringFilter sf;
                sf.SetFile(plan_reader.GetFile());

                // Patient information
                if (plan_ds.FindDataElement(gdcm::Tag(0x0010, 0x0010))) {
                    dcm.patient_name = sf.ToString(gdcm::Tag(0x0010, 0x0010));
                }
                if (plan_ds.FindDataElement(gdcm::Tag(0x0010, 0x0020))) {
                    dcm.patient_id = sf.ToString(gdcm::Tag(0x0010, 0x0020));
                }
                if (plan_ds.FindDataElement(gdcm::Tag(0x0010, 0x0030))) {
                    dcm.patient_birth_date = sf.ToString(gdcm::Tag(0x0010, 0x0030));
                }
                if (plan_ds.FindDataElement(gdcm::Tag(0x0010, 0x0040))) {
                    dcm.patient_sex = sf.ToString(gdcm::Tag(0x0010, 0x0040));
                }

                // Study/Series information
                if (plan_ds.FindDataElement(gdcm::Tag(0x0020, 0x000D))) {
                    dcm.study_instance_uid = sf.ToString(gdcm::Tag(0x0020, 0x000D));
                }
                if (plan_ds.FindDataElement(gdcm::Tag(0x0020, 0x000E))) {
                    dcm.series_instance_uid = sf.ToString(gdcm::Tag(0x0020, 0x000E));
                }
                if (plan_ds.FindDataElement(gdcm::Tag(0x0020, 0x0052))) {
                    dcm.frame_of_reference_uid = sf.ToString(gdcm::Tag(0x0020, 0x0052));
                }

                // Date/Time information
                if (plan_ds.FindDataElement(gdcm::Tag(0x0008, 0x0021))) {
                    dcm.series_date = sf.ToString(gdcm::Tag(0x0008, 0x0021));
                }
                if (plan_ds.FindDataElement(gdcm::Tag(0x0008, 0x0023))) {
                    dcm.content_date = sf.ToString(gdcm::Tag(0x0008, 0x0023));
                }
                if (plan_ds.FindDataElement(gdcm::Tag(0x0008, 0x0031))) {
                    dcm.series_time = sf.ToString(gdcm::Tag(0x0008, 0x0031));
                }
                if (plan_ds.FindDataElement(gdcm::Tag(0x0008, 0x0033))) {
                    dcm.content_time = sf.ToString(gdcm::Tag(0x0008, 0x0033));
                }

                // Institution and Physician
                if (plan_ds.FindDataElement(gdcm::Tag(0x0008, 0x0080))) {
                    dcm.institution_name = sf.ToString(gdcm::Tag(0x0008, 0x0080));
                }
                if (plan_ds.FindDataElement(gdcm::Tag(0x0008, 0x0090))) {
                    dcm.referring_physician = sf.ToString(gdcm::Tag(0x0008, 0x0090));
                }
                if (plan_ds.FindDataElement(gdcm::Tag(0x0008, 0x103E))) {
                    dcm.series_description = sf.ToString(gdcm::Tag(0x0008, 0x103E));
                }

                // SOP information
                if (plan_ds.FindDataElement(gdcm::Tag(0x0008, 0x0016))) {
                    dcm.sop_class_uid = sf.ToString(gdcm::Tag(0x0008, 0x0016));
                }
                if (plan_ds.FindDataElement(gdcm::Tag(0x0008, 0x0018))) {
                    dcm.sop_instance_uid = sf.ToString(gdcm::Tag(0x0008, 0x0018));
                    dcm.referenced_rt_plan_sop_instance_uid = dcm.sop_instance_uid;
                }

                // Dose-specific information
                if (plan_ds.FindDataElement(gdcm::Tag(0x3004, 0x0004))) {
                    dcm.dose_type = sf.ToString(gdcm::Tag(0x3004, 0x0004));
                }
                if (plan_ds.FindDataElement(gdcm::Tag(0x3004, 0x0014))) {
                    dcm.tissue_heterogeneity_correction = sf.ToString(gdcm::Tag(0x3004, 0x0014));
                }

                std::cout << "RT Plan header information successfully loaded." << std::endl;
            } else {
                std::cout << "Warning: Could not read RT Plan header information." << std::endl;
            }
        } else
        {
            dcm.plan_name = "";
        ...
        if (this->debug_mode) std::cout << "Reading DICOM directory.. : Loading RT Ion plan data from " << dcm.plan_name << " .." << std::endl;
    ...
    read_logfile_dir(int beamNumber, const std::string& beamName)
    {
        logfiles_t logFileData;

        std::cout << "---------------------------------------------------------------------------" << std::endl;
        std::cout << "Starting process of loading log files.." << std::endl;
        std::cout << "RT Plan Beam #" << beamNumber << " - Field Name: '" << beamName << "'" << std::endl;
        std::cout << "---------------------------------------------------------------------------" << std::endl;
        ...
        // **핵심 변경: Name-based matching with {digit}G{angle} pattern extraction**
        std::string matchedFolder = "";

        std::cout << "Available log folders:" << std::endl;
        for (const auto& folder : fileFolderName) {
            std::cout << "  - " << std::filesystem::path(folder).filename().string() << std::endl;
        }

        // Extract {digit}G{angle} pattern from RT Plan beam name
        // Pattern: one or more digits + 'G' + exactly 3 digits
        std::regex beam_pattern(R"((\d+G\d{3}))");
        std::smatch beam_match;
        std::string extractedBeamPattern = "";

        if (std::regex_search(beamName, beam_match, beam_pattern)) {
            extractedBeamPattern = beam_match[1].str();
            std::cout << "Extracted beam pattern from RT Plan: '" << extractedBeamPattern
                      << "' (from '" << beamName << "')" << std::endl;
        } else {
            std::cout << "Warning: Could not extract {digit}G{angle} pattern from beam name: "
                      << beamName << std::endl;
            extractedBeamPattern = beamName;  // Fallback to original name
        }

        for (const auto& folderPath : fileFolderName)
        {
            std::filesystem::path p(folderPath);
            std::string folderBaseName = p.filename().string();

            // Extract {digit}G{angle} pattern from log folder name
            std::string extractedFolderPattern = "";
            if (std::regex_search(folderBaseName, beam_match, beam_pattern)) {
                extractedFolderPattern = beam_match[1].str();
            } else {
                // Fallback: remove "_TX" suffix if present
                if (folderBaseName.length() > 3 &&
                    folderBaseName.substr(folderBaseName.length() - 3) == "_TX") {
                    extractedFolderPattern = folderBaseName.substr(0, folderBaseName.length() - 3);
                } else {
                    extractedFolderPattern = folderBaseName;
                }
            }

            // Match: RT Plan beam pattern == folder pattern
            if (extractedFolderPattern == extractedBeamPattern) {
                matchedFolder = folderPath;
                std::cout << "✓ Exact match found - Log folder: " << p.filename().string()
                          << " (pattern: " << extractedFolderPattern << ") matches RT Plan beam: "
                          << beamName << " (pattern: " << extractedBeamPattern << ")" << std::endl;
                break;
            }

            // Fallback: Partial match (folder pattern contains beam pattern)
            if (matchedFolder.empty() && extractedFolderPattern.find(extractedBeamPattern) != std::string::npos) {
                matchedFolder = folderPath;
                std::cout << "✓ Partial match found - Log folder: " << p.filename().string()
                          << " (pattern: " << extractedFolderPattern << ") contains RT Plan beam pattern: "
                          << extractedBeamPattern << std::endl;
            }
        }

        // Error handling (fail-fast approach)
        if (matchedFolder.empty()) {
            std::cerr << "ERROR: No matching log folder found for RT Plan Beam #" << beamNumber
                      << " (Field Name: " << beamName << ")" << std::endl;
            std::cerr << "Available log folders:" << std::endl;
            for (const auto& folder : fileFolderName) {
                std::cerr << "  - " << std::filesystem::path(folder).filename().string() << std::endl;
            }
            throw std::runtime_error("No matching log folder found for RT Plan Beam #" +
                                     std::to_string(beamNumber) + " (Field Name: " + beamName + ")");
        }

        // **기존 CSV 파싱 로직 그대로 사용 (i == beamIndex 조건 제거)**
        std::filesystem::path p(matchedFolder);
        ...
        for (auto &filename : sortedByName)
        {
            if (filename.extension() == ".csv")
            {
                ...
                logBeamEnergyString = logBeamEnergyString.substr(logBeamEnergyString.find('_') + 1,
                                                                  logBeamEnergyString.find('M') - 2);
                float layerBeamEnergy = std::stof(logBeamEnergyString);
                fieldBeamEnergy.push_back(layerBeamEnergy);

                if (this->debug_mode) {
                    std::cout << "Reading log file information in the field directory.. : " + filename.string() + ".." <<
                    std::endl;
                }
        ...
        std::cout << "✓ Successfully loaded log files for RT Plan Beam #" << beamNumber
                  << " (Field Name: " << beamName << ")" << std::endl;
    ...
    setup_world() {
        ...
        if (this->debug_mode) {
            ...
            std::cout << "Creating child in world geometry: " << std::endl;
            std::cout << "  Phantom size: (x, y, z)->";
            if (this->twoCentimeterMode) std::cout << "(400, 400, 2)" << std::endl;
            else std::cout << "(" << this->dcm_.dim_.x << ", " << this->dcm_.dim_.y << ", " << this->dcm_.dim_.z << ")" << std::endl;
        }
        ...
        if (!this->usingPhantomGeo)
        {
            ...
            if (this->debug_mode) std::cout << "Creating material information for grid.." << std::endl;
    ...
    setup_beamsource() {
        ...
        this->sid = temp_sid[0];
        if (this->debug_mode) std::cout << "Creating beam source object.. : Detected snout position --> " << this->sid << " mm" << std::endl;
        else std::cout << "snout position : " << this->sid << " mm" << std::endl;
        mqi::coordinate_transform<R> p_coord = this->tx->get_coordinate(bnb);
        p_coord.angles[3]                    = 90.0;   //iec2dicom angle
        ...
        if (this->debug_mode) {
            ...
            std::cout << "Creating coordinate transform for beam source " << std::endl;
            std::cout << "p_coord angle: " << std::endl;
            std::cout << "  (" << p_coord.angles[0] << ", " << p_coord.angles[1] << ", " << p_coord.angles[2] << ", " << p_coord.angles[3] << ")" << std::endl;
        }
        ...
        logfiles_t logFileData = read_logfile_dir(bnb, beam_names[bnb - 1]);
        ...
        if (this->debug_mode) std::cout << "Creating beam source complete!" << std::endl;
    }
        ...
        for (int c_ind = 0; c_ind < this->world->n_children; c_ind++) {
            for (int s_ind = 0; s_ind < this->world->children[c_ind]->n_scorers; s_ind++) {
                ...
                if (!this->output_format.compare("mhd")) {
                    ...
                    // Prepare DICOM header information from RT Plan
                    mqi::io::dcm_header_info header_info;
                    header_info.patient_name = this->dcm_.patient_name;
                    header_info.patient_id = this->dcm_.patient_id;
                    header_info.patient_birth_date = this->dcm_.patient_birth_date;
                    header_info.patient_sex = this->dcm_.patient_sex;
                    header_info.study_instance_uid = this->dcm_.study_instance_uid;
                    header_info.series_instance_uid = this->dcm_.series_instance_uid;
                    header_info.frame_of_reference_uid = this->dcm_.frame_of_reference_uid;
                    header_info.series_date = this->dcm_.series_date;
                    header_info.content_date = this->dcm_.content_date;
                    header_info.series_time = this->dcm_.series_time;
                    header_info.content_time = this->dcm_.content_time;
                    header_info.institution_name = this->dcm_.institution_name;
                    header_info.referring_physician = this->dcm_.referring_physician;
                    header_info.series_description = this->dcm_.series_description;
                    header_info.dose_type = this->dcm_.dose_type;
                    header_info.tissue_heterogeneity_correction = this->dcm_.tissue_heterogeneity_correction;
                    header_info.referenced_rt_plan_sop_instance_uid = this->dcm_.referenced_rt_plan_sop_instance_uid;

                    // Save to DCM with full header information
                    mqi::io::save_to_dcm<R>(
                        this->world->children[c_ind]->scorers[s_ind],
                        this->world->children[c_ind],  // geometry_node
